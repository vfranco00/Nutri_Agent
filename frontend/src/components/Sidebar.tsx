...
import { Home, ChefHat, LogOut, Moon, Sun, ArrowLeft, ShoppingCart, UserIcon, Book, CheckCheck, Shield, CalendarRange, Crown } from 'lucide-react';
...
export function Sidebar({ toggleTheme, isDark }: SidebarProps) {
  ...
  const menuItems = [
    ...
    { name: 'Meu Perfil', icon: UserIcon, path: '/profile', color: 'text-green-500' },
    { name: 'Minhas Receitas', icon: Book, path: '/recipes', color: 'text-orange-500' },
    ...
    { name: 'Assinatura', icon: Crown, path: '/planos', color: 'text-amber-500' },
  ...
  return (
    <aside className="w-20 lg:w-64 bg-white dark:bg-zinc-900 border-r border-zinc-200 dark:border-zinc-800 flex flex-col transition-all duration-300 fixed h-full z-20">
      ...
      <div className="h-16 flex items-center justify-center lg:justify-start lg:px-6 border-b border-zinc-200 dark:border-zinc-800">
        <img src="/nutri-agent-logo-horizontal.png" alt="NutriAgent" className="h-10 w-auto object-contain" />
      </div>
