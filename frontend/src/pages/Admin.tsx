...
export function Admin() {
  ...
  async function handleDelete(id: number) {
    if (
      !confirm(
        "Tem certeza que deseja BANIR este usuário? Essa ação deleta tudo dele."
      )
    )
      return;
    try {
  ...
  if (loading)
    return (
      <div className="p-20 text-center text-zinc-500">Carregando painel...</div>
    );
  ...
  return (
    <div className="max-w-6xl mx-auto">
      <button
        ...
        className="mb-6 flex items-center gap-2 text-zinc-500 hover:text-zinc-800 dark:hover:text-white transition-colors"
      >
          ...
          <table className="w-full text-sm text-left">
            ...
            <tbody>
              {users.map((user) => (
                <tr
                  key={user.id}
                  className="bg-white dark:bg-zinc-900 border-b border-zinc-100 dark:border-zinc-800 hover:bg-zinc-50 dark:hover:bg-zinc-800/50 transition-colors"
                >
                  ...
                  <td className="px-6 py-4 text-center">
                    {user.is_superuser ? (
                      <span className="bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-400 text-xs px-2 py-1 rounded-full font-bold border border-red-200 dark:border-red-800">
                        ADMIN
                      </span>
                    ) : (
                      <span className="bg-zinc-100 dark:bg-zinc-800 text-zinc-500 text-xs px-2 py-1 rounded-full">
                        USER
                      </span>
                    )}
                  </td>
