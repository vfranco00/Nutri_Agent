...
import { Plus, Clock, Flame, ChefHat, Trash2, Loader2, X, Edit2, Save, AlertCircle } from 'lucide-react';
...
export function Recipes() {
  ...
  function handleOpenRecipe(recipe: Recipe) {
    ...
    setEditForm({
      title: recipe.title, instructions: recipe.instructions, prep_time: recipe.prep_time || 0, calories: recipe.calories || 0
    });
    setIsEditing(false);
  }
  ...
  async function handleSaveEdit() {
    ...
    try {
      ...
      setRecipes(recipes.map(r => r.id === selectedRecipe.id ? res.data : r));
  ...
  return (
    <>
      <div className="flex items-center justify-between mb-8">
        ...
        <button onClick={() => navigate('/recipes/new')} className="bg-orange-600 hover:bg-orange-700 text-white px-4 py-2 rounded-lg font-medium flex items-center gap-2 shadow-sm">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {recipes.map((recipe) => (
          <div key={recipe.id} onClick={() => handleOpenRecipe(recipe)} className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl p-5 hover:border-orange-500/50 hover:-translate-y-1 transition-all cursor-pointer group shadow-sm">
            <h3 className="text-lg font-bold dark:text-zinc-100 group-hover:text-orange-500 transition-colors line-clamp-1 mb-3">{recipe.title}</h3>
            <p className="text-zinc-500 dark:text-zinc-400 text-sm line-clamp-3 mb-4 h-12">{recipe.instructions}</p>
          ...
          <div className="bg-white dark:bg-zinc-900 w-full max-w-2xl rounded-2xl shadow-2xl flex flex-col max-h-[90vh]">
            ...
            <div className="p-6 overflow-y-auto space-y-6">
              <div className="flex gap-4">
                <div className="flex items-center gap-2 text-zinc-500 dark:text-zinc-400 bg-zinc-50 dark:bg-zinc-950 px-3 py-2 rounded-lg border border-zinc-200 dark:border-zinc-800">
              ...
              <div className="space-y-2">
                ...
                {isEditing ? (
                  <textarea rows={8} className="w-full bg-zinc-50 dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-lg p-3 dark:text-zinc-300 outline-none resize-none" value={editForm.instructions} onChange={e => setEditForm({...editForm, instructions: e.target.value})} />
                ) : (
                  <p className="text-zinc-600 dark:text-zinc-300 leading-relaxed whitespace-pre-wrap">{selectedRecipe.instructions}</p>
                )}
              </div>
              ...
              {!isEditing && (
                <div className="bg-yellow-50 dark:bg-yellow-500/10 border border-yellow-200 dark:border-yellow-500/20 p-4 rounded-lg flex gap-3 items-start">
                  ...
                  <p className="text-yellow-700 dark:text-yellow-200/70 text-sm">Edição de ingredientes complexos em breve.</p>
                </div>
              )}
            </div>
