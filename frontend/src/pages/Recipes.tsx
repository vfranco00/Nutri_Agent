...
import { type Recipe, CATEGORIES, type User } from "../types";
import {
  Plus,
  Clock,
  Flame,
...
export function Recipes() {
  ...
  // --- LOGICA DE RECALCULO DE CALORIAS ---
  function recalculateTotal(ingredientsList: any[]) {
  ...
  function updateIngredient(
    ...
    if (field === "calories") {
      const newTotal = recalculateTotal(newIngs);
      setEditForm({ ...editForm, ingredients: newIngs, calories: newTotal });
    } else {
  ...
  function handleSpeak() {
    ...
    }. Modo de preparo: ${cleanMarkdown(selectedRecipe.instructions)}`;
                  ...
                  <button
                    ...
                    className={`p-2 rounded-full transition-colors ${
                      isSpeaking
                        ...
                        : "bg-zinc-100 dark:bg-zinc-800 text-zinc-500 dark:text-zinc-400 hover:text-orange-500"
                    }`}
                  >
                    {isSpeaking ? (
                      ...
                      editForm.ingredients.map((ing, i) => (
                        <li key={i} className="flex gap-2 items-center">
                          ...
                          <input
                            ...
                            className="w-16 bg-zinc-50 dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded px-2 py-1 text-sm dark:text-white text-right"
                          />
                          <button
