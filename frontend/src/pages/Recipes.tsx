...
import { Plus, Clock, Flame, ChefHat, Trash2, Loader2, X, Edit2, Save, Carrot, Volume2, Square, Star, Target, Trophy, Bot, Crown } from 'lucide-react';
...
export function Recipes() {
  ...
  const [editForm, setEditForm] = useState<EditFormData>({
    title: '', instructions: '', prep_time: 0, calories: 0, preparation_method: '', category: 'almoco', ingredients: []
  });
  ...
  async function handleOpenRecipe(recipe: Recipe) {
    ...
    setEditForm({
      ...
      // @ts-ignore
      ingredients: recipe.ingredients ? recipe.ingredients.map(i => ({ name: i.name, quantity: i.quantity, unit: i.unit, calories: i.calories })) : []
    });
  ...
  function addIngredient() {
    setEditForm({ ...editForm, ingredients: [...editForm.ingredients, { name: '', quantity: 1, unit: 'un', calories: 0 }] });
  }
    ...
    <>
      <div className="flex flex-col gap-6 mb-8">
        ...
        <div className="flex gap-1 bg-zinc-100 dark:bg-zinc-900 p-1 rounded-lg w-fit">
          <button onClick={() => setViewMode('mine')} className={`px-4 py-2 text-sm font-medium rounded-md transition-all ${viewMode === 'mine' ? 'bg-white dark:bg-zinc-800 text-orange-600 shadow-sm' : 'text-zinc-500 hover:text-zinc-800 dark:text-zinc-400'}`}>Meus Pratos</button>
          <button onClick={() => setViewMode('community')} className={`px-4 py-2 text-sm font-medium rounded-md transition-all ${viewMode === 'community' ? 'bg-white dark:bg-zinc-800 text-blue-600 shadow-sm' : 'text-zinc-500 hover:text-zinc-800 dark:text-zinc-400'}`}>Comunidade</button>
        </div>
        ...
        {viewMode === 'community' && leaderboard.length > 0 && (
          <div className="bg-gradient-to-r from-blue-900 to-purple-900 rounded-xl p-4 mb-2 shadow-lg animate-fadeIn text-white border border-blue-800">
            <h3 className="text-sm font-bold uppercase tracking-wider mb-3 flex items-center gap-2 text-yellow-400"><Trophy className="h-4 w-4"/> Top Chefs da Semana</h3>
            <div className="flex gap-4 overflow-x-auto pb-2 scrollbar-hide">
              {leaderboard.map((user, idx) => (
                <div key={user.id} className="flex items-center gap-2 bg-white/10 px-3 py-2 rounded-lg shrink-0">
                  <div className="font-bold text-lg text-yellow-400 w-6">#{idx + 1}</div>
                  <div>
                    <span className="block text-sm font-semibold truncate max-w-[100px]">{user.full_name?.split(' ')[0]}</span>
                    <span className="text-xs text-white/60">{user.score} XP</span>
                  </div>
                  {idx === 0 && <Crown className="h-4 w-4 text-yellow-400 ml-1"/>}
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="flex gap-2 overflow-x-auto pb-2 md:pb-0 scrollbar-hide">
          {Object.entries(CATEGORIES).map(([key, label]) => (
            <button key={key} onClick={() => setFilter(key)} className={`px-4 py-2 rounded-full text-sm font-medium transition-colors whitespace-nowrap ${filter === key ? 'bg-orange-500 text-white' : 'bg-white dark:bg-zinc-900 text-zinc-500 dark:text-zinc-400 border border-zinc-200 dark:border-zinc-800 hover:border-orange-500'}`}>{label}</button>
          ))}
        </div>
      </div>
      ...
      {viewMode === 'mine' && recommendations.length > 0 && !loading && (
        <div className="mb-10 animate-fadeIn">
          <h2 className="text-xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-orange-500 to-pink-500 mb-4 flex items-center gap-2"><Target className="h-5 w-5 text-orange-500" /> Escolhidas para Você</h2>
          <div className="flex gap-4 overflow-x-auto pb-4 scrollbar-hide snap-x">
            {recommendations.map((recipe) => (
              <div key={recipe.id} onClick={() => handleOpenRecipe(recipe)} className="min-w-[280px] bg-white dark:bg-zinc-900 border border-orange-100 dark:border-orange-900/30 rounded-xl p-4 cursor-pointer hover:scale-[1.02] transition-transform snap-center shadow-sm">
                <div className="flex justify-between items-start mb-2">
                  <span className="bg-orange-100 dark:bg-orange-900/30 text-orange-600 dark:text-orange-300 text-[10px] font-bold px-2 py-1 rounded uppercase truncate max-w-[120px]">{recipe.category || 'Geral'}</span>
                  <span className="text-xs text-zinc-400 flex items-center gap-1 shrink-0"><Flame className="h-3 w-3" /> {Math.round(recipe.calories || 0)}</span>
                </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {sortedRecipes.map((recipe) => (
          <div key={recipe.id} onClick={() => handleOpenRecipe(recipe)} className="relative bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl p-5 hover:border-orange-500/50 hover:-translate-y-1 transition-all cursor-pointer group shadow-sm">
            {recipe.is_new && viewMode === 'mine' && <div className="absolute -top-2 -left-2 bg-green-500 text-white text-[10px] font-bold px-2 py-1 rounded-full shadow-lg z-20 animate-bounce">NOVA</div>}
            
            {/* Tag IA */}
            {/* @ts-ignore */}
            {recipe.is_ai && <div className="absolute top-4 left-4 bg-blue-500/10 text-blue-500 text-[10px] font-bold px-2 py-1 rounded border border-blue-500/20 flex items-center gap-1"><Bot className="h-3 w-3"/> IA</div>}
            ...
            <div className="flex items-center justify-between p-6 border-b border-zinc-200 dark:border-zinc-800">
              {isEditing ? (
                ...
                <div className="flex items-center gap-3">
                  ...
                  <button onClick={handleSpeak} className={`p-2 rounded-full transition-colors ${isSpeaking ? 'bg-orange-500 text-white animate-pulse' : 'bg-zinc-100 dark:bg-zinc-800 text-zinc-500 dark:text-zinc-400 hover:text-orange-500'}`}>{isSpeaking ? <Square className="h-4 w-4 fill-current" /> : <Volume2 className="h-5 w-5" />}</button>
                </div>
              )}
            ...
            <div className="p-6 overflow-y-auto space-y-6">
              ...
              <div className="space-y-3">
                <div className="flex justify-between items-center">
                   ...
                   {isEditing && <button onClick={addIngredient} className="text-xs text-green-500 flex items-center gap-1 hover:text-green-400"><Plus className="h-3 w-3"/> Adicionar</button>}
                </div>
                  ...
                  {isEditing ? (
                    ...
                    editForm.ingredients.map((ing, i) => (
                      <li key={i} className="flex gap-2 items-center">
                        <input value={ing.name} onChange={e => updateIngredient(i, 'name', e.target.value)} className="flex-1 bg-zinc-50 dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded px-2 py-1 text-sm dark:text-white" placeholder="Nome" />
                        <input type="number" value={ing.quantity} onChange={e => updateIngredient(i, 'quantity', e.target.value)} className="w-16 bg-zinc-50 dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded px-2 py-1 text-sm dark:text-white" placeholder="Qtd" />
                        <input value={ing.unit} onChange={e => updateIngredient(i, 'unit', e.target.value)} className="w-16 bg-zinc-50 dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded px-2 py-1 text-sm dark:text-white" placeholder="Un" />
                        <button onClick={() => removeIngredient(i)} className="text-red-500 hover:text-red-400"><Trash2 className="h-4 w-4"/></button>
                      </li>
                    ))
                  ) : (
                    // MODO VISUALIZAÇÃO
                    // @ts-ignore
                    selectedRecipe.ingredients && selectedRecipe.ingredients.map((ing) => (
                  ...
                  {viewMode === 'mine' && (
                    <>
                      ...
                      {!selectedRecipe.is_ai ? (
                        ...
                        <span className="text-zinc-400 text-xs flex items-center gap-1 cursor-not-allowed select-none" title="Gerado por IA (Protegido)"><Bot className="h-4 w-4"/> Gerada por IA</span>
                      )}
